...
func TestParameterChangeProposal(t *testing.T) {
	...
	pcp := NewParameterChangeProposal("test title", "test description", []ParamChange{pc1, pc2}, false)
	...
	pcp = NewParameterChangeProposal("test title", "test description", []ParamChange{pc3}, false)
	require.Error(t, pcp.ValidateBasic())

	pc4 := NewParamChange("sub", "", "cat")
	pcp = NewParameterChangeProposal("test title", "test description", []ParamChange{pc4}, false)
	require.Error(t, pcp.ValidateBasic())
}
