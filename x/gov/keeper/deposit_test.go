...
func TestDeposits(t *testing.T) {
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			TestAddrs := simapp.AddTestAddrsIncremental(app, ctx, 2, sdk.NewInt(40000000))

			tp := TestProposal
			proposal, err := app.GovKeeper.SubmitProposal(ctx, tp, "", tt.isExpedited)
			...
			if tt.isExpedited {
				fourStake = sdk.NewCoins(sdk.NewCoin(sdk.DefaultBondDenom, app.StakingKeeper.TokensFromConsensusPower(ctx, 4).Mul(sdk.NewInt(2))))
				fiveStake = sdk.NewCoins(sdk.NewCoin(sdk.DefaultBondDenom, app.StakingKeeper.TokensFromConsensusPower(ctx, 5).Mul(sdk.NewInt(2))))
			}
